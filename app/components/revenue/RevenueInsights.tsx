...
import type { RevenueInsight } from "../../revenue/mockRevenue";

const iconMap: Record<RevenueInsight["type"], LucideIcon> = {
...
const colorMap: Record<RevenueInsight["type"], { bg: string; icon: string; trend: string }> = {
...
type RevenueInsightsProps = {
  items: RevenueInsight[];
};

export function RevenueInsights({ items }: RevenueInsightsProps) {
  return (
    <section className="rounded-[1.6rem] border border-slate-200 bg-white p-4 shadow-[0_10px_35px_rgba(15,23,42,0.06)] sm:p-5">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold text-slate-900">Revenue Insights</p>
          <p className="text-sm text-slate-500">AI-powered revenue observations</p>
        </div>
      </div>
